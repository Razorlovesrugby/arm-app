...
## 🔄 As-Built Deviations & Post-Implementation Fixes

### Deviation 1: Capacity Check 3 — toast system replaced `setError`

**Spec said:** Capacity Check 3 should call `flashError()` + `setError('Team is full (maximum squad size reached)')`.

**Problem discovered:** The `error` state in `useSelectionBoard` is shared between fatal data-load errors and transient action errors. In `SelectionBoard.tsx`, ALL board content is gated behind `!error` (lines 962–974). Calling `setError(...)` caused the entire player list to disappear — leaving only the error text on screen.

**What was actually built:** Replaced `flashError()` + `setError()` with a new dedicated `flashToast(message)` pattern:

- Added `const [toast, setToast] = useState<string | null>(null)` to `useSelectionBoard.ts`
- Added `function flashToast(message: string)` — sets `toast`, auto-clears after 1s via `setTimeout`
- Capacity Check 3 now calls `flashToast('Team is full (maximum squad size reached)')` and returns — `error` state is never touched
- `toast` is exposed on the `UseSelectionBoardReturn` interface

---

### Deviation 2: TypeScript build error — `toast` missing from `UseSelectionBoardReturn`

**Problem:** First build after the toast change failed with:
```
error TS2339: Property 'toast' does not exist on type 'UseSelectionBoardReturn'
error TS2353: Object literal may only specify known properties, and 'toast' does not exist in type 'UseSelectionBoardReturn'
```

**Fix:** Added `toast: string | null` to the `UseSelectionBoardReturn` interface (line ~41 of `useSelectionBoard.ts`). Committed separately as hotfix `f56a601`.

---

### Deviation 3: Toast UX — component-side wiring

The existing `SelectionBoard.tsx` already had a `toastMessage` state + `position: fixed` pill UI used for clipboard copy notifications. Rather than build a second toast system, the hook's `toast` is piped into the existing one:

```tsx
// Pipe hook-level toasts into the existing toast UI
useEffect(() => {
  setToastMessage(toast)   // clears when toast → null after 1s
}, [toast])

// Dismiss on any screen tap
useEffect(() => {
...
**Key behaviours:**
- Auto-dismisses after **1 second** (spec did not specify a duration)
- Tapping **anywhere on screen** dismisses it immediately (`{ once: true }` on a document-level listener)
- The toast does NOT replace or hide any board content

---

### Updated Acceptance Criteria

- [x] Dragging a player onto another filled player slot does nothing (snaps back)
- [x] Dragging a player onto an empty "Drop here" slot successfully places them there
- [x] Clicking "+" adds players sequentially until `default_squad_size` is reached
- [x] Clicking "+" backfills the first null gap when team is at capacity
- [x] Clicking "+" when team is completely full shows a toast popup ("Team is full (maximum squad size reached)"), auto-dismisses in 1s, dismisses on any tap, and board remains fully visible throughout
...
| Hash | Description |
|------|-------------|
| `a69ea61` | fix: Phase 20.1.3 — Selection Board drag & drop and capacity fixes (original spec) |
| `f7c676c` | fix: show team-full error as toast popup without hiding the board |
| `f56a601` | fix: add toast to UseSelectionBoardReturn interface (build error hotfix) |
| `b2a6b7e` | fix: toast now auto-dismisses and clears on tap |
| `620f52c` | fix: toast dismisses on any screen tap and auto-hides after 1s |

---

*Spec completed: 2026-04-30*
