...
- [ ] PDF layout dynamically adjusts row height or pagination to prevent 'dead space' when squad size is significantly smaller than default 23
...
- [ ] Shows "Copying not supported in this browser" when Clipboard API unavailable
...
1. **Async Clipboard:** `navigator.clipboard` errors handled with fallback toast message and graceful degradation
2. **Empty Teams:** Export buttons only show when teams exist
3. **No Kicking Data:** Shows `—` when `totalAttempts === 0`, shows "Loading..." while fetching
4. **Mobile Safari:** Toast positioned with `fixed` and `transform` for centering
5. **PDF Row Mapping:** Dynamically calculates rows based on `squadSize` not hardcoded 23, adjusts layout to prevent dead space
6. **WhatsApp Numbering:** Bench numbering starts at `starters.length + 1` (dynamic, not hardcoded 16)
7. **Clipboard API Unavailable:** Shows "Copying not supported in this browser" on non-HTTPS or older browsers
8. **Supabase Performance:** Kicking % fetch includes loading state for slow network connections
