-- Migration 026: Fix RDO seed — re-runs 024 now that profiles.club_id is nullable (025)
...
-- Skip if already seeded (clubs are the reliable idempotency marker here)
IF EXISTS (SELECT 1 FROM clubs WHERE name = 'College Rifles Prems') THEN
    RAISE NOTICE 'RDO seed already applied, skipping.';
    RETURN;
END IF;
...
-- ─────────────────────────────────────────
-- Only insert profile if the auth user actually exists (avoids FK violation in local envs)
IF EXISTS (SELECT 1 FROM auth.users WHERE id = rdo_uuid) THEN
...
ELSE
    RAISE NOTICE 'Auth user % not found — skipping profile and rdo_club_access seed.', rdo_uuid;
END IF;
...
SELECT c_prems, 'PremPlayer ' || gs, 'prem' || gs || '@test.com', '555-01' || gs, '1998-05-15',
...
INSERT INTO players (club_id, name, email, phone, date_of_birth, primary_position, player_type, status)
SELECT c_womens, 'WomensPlayer ' || gs, 'w' || gs || '@test.com', '555-02' || gs, '1999-08-22',
    (ARRAY['Prop','Hooker','Lock','Flanker','Number 8','Scrum-half','Fly-half','Centre','Wing','Fullback'])[floor(random()*10+1)],
    'Women''s',
...
INSERT INTO players (club_id, name, email, phone, date_of_birth, primary_position, player_type, status)
SELECT c_u85, 'RaiderPlayer ' || gs, 'u85' || gs || '@test.com', '555-03' || gs, '1995-02-10',
    (ARRAY['Prop','Hooker','Lock','Flanker','Number 8','Scrum-half','Fly-half','Centre','Wing','Fullback'])[floor(random()*10+1)],
    'Open',
...
INSERT INTO players (club_id, name, email, phone, date_of_birth, primary_position, player_type, status)
SELECT c_colts, 'ColtPlayer ' || gs, 'colt' || gs || '@test.com', '555-04' || gs, '2004-11-05',
    (ARRAY['Prop','Hooker','Lock','Flanker','Number 8','Scrum-half','Fly-half','Centre','Wing','Fullback'])[floor(random()*10+1)],
    'Performance',
    CASE WHEN random() < 0.05 THEN 'Injured' ELSE 'Active' END
...
SELECT w_curr_u85, c_u85, p.id,
    CASE WHEN random() < 0.40 THEN 'Available' WHEN random() < 0.60 THEN 'TBC' ELSE 'Unavailable' END
