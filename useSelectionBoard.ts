...
export interface UseSelectionBoardReturn {
  teams: SelectionTeam[]           // only visible=true teams, ordered by name
  allPlayers: Player[]
...
export function useSelectionBoard(weekId: string | null): UseSelectionBoardReturn {
  ...
  const assignPlayer = useCallback(async (teamId: string, playerId: string) => {
    ...
    if (targetSel) {
      ...
      // No existing selection row for this team — add a synthetic one for optimistic UI
      newSelections = [
        ...newSelections,
        {
    ...
    try {
      // Remove from old team first (if applicable)
      if (existing) {
  ...
  const removePlayer = useCallback(async (teamId: string, playerId: string) => {
    ...
    // If removed player was captain, clear captain_id
